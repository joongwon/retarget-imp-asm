...
let map2 f o1 o2 =
  match (o1, o2) with Some x1, Some x2 -> Some (f x1 x2) | _ -> None

module Val = struct
  type bound = Inf | Fin of int
  type t = Intv of bound * bound | Bot (* empty *)
...
module Mem = struct
  ...
  let widen (mold : t) (mnew : t) : t =
    ...
    | None, m | m, None -> m
    | Some m1, Some m2 -> Some (List.map2 Val.widen m1 m2)
...
let encode : T.Pgm.t * Val.t -> Mem.t =
 fun (prog, input) ->
  ...
  in
  let code = encode_p prog |> List.map Val.abstract in
  Some (Val.abstract 2 :: input :: code)

let decode : Mem.t -> Val.t =
 fun m -> match m with Some (_ :: v :: _) -> v | _ -> Val.bot
